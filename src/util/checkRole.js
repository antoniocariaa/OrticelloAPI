...
const checkRole = (allowedTypes, requireAdmin = false) => {
    return (req, res, next) => {
        ...
        if (!req.loggedUser) {
            return res.status(401).json({ error: "Accesso negato. Utente non autenticato." });
        }
        ...
        if (!allowedTypes.includes(user.tipo)) {
            return res.status(403).json({ 
                error: "Accesso proibito. Il tuo ruolo non permette questa operazione." 
            });
        }
        ...
        if (requireAdmin && !user.admin) {
            return res.status(403).json({ 
                error: "Accesso proibito. Sono richiesti privilegi di amministratore." 
            });
        }
