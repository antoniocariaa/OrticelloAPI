...
- 🇬🇧 Inglese (en)
- 🇩🇪 Tedesco (de)
- 🇫🇷 Francese (fr) - [Da implementare]
- 🇪🇸 Spagnolo (es) - [Da implementare]

## Come Funziona

Il sistema rileva automaticamente la lingua preferita dell'utente in questo ordine di priorità:
...
├── en.json (Inglese)
├── de.json (Tedesco)
├── fr.json (Francese) - Da completare
└── es.json (Spagnolo) - Da completare
```
...
- Il middleware `detectLanguage` è applicato globalmente in `app.js`
- La funzione `req.t()` è disponibile in tutti i controller
...
- [ ] Completare traduzioni francesi (fr.json)
- [ ] Completare traduzioni spagnole (es.json)
- [ ] Aggiungere traduzioni per tutti i controller
- [ ] Implementare traduzioni per messaggi di validazione Mongoose
- [ ] Aggiungere supporto per plurali
